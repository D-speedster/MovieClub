...
import {
  ...
  Button, Alert, CircularProgress, Stack, Card, CardContent, Chip,
} from '@mui/material';
import {
  Save as SaveIcon,
  Movie as MovieIcon,
...
export default function MovieSettings() {
  ...
  useEffect(() => {
    ApiRequest.get('/settings/movie_page')
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await ApiRequest.post('/settings/movie_page', { value: settings, description: 'تنظیمات صفحه فیلم‌ها' });
  ...
  return (
    <Box sx={{ direction: 'rtl' }}>
      ...
      <Card sx={sx.card}>
        <CardContent>
          <Typography sx={sx.sectionTitle}><VisibilityIcon fontSize="small" sx={{ color: '#e50914' }} />تنظیمات نمایش</Typography>
          <Stack spacing={1}>
            {[
              { key: 'showRating',  label: 'نمایش امتیاز IMDb' },
              { key: 'showYear',    label: 'نمایش سال ساخت' },
              { key: 'showGenre',   label: 'نمایش ژانر' },
              { key: 'showDuration',label: 'نمایش مدت زمان فیلم' },
            ].map(({ key, label }) => (
              <FormControlLabel
                key={key}
                control={<Switch checked={settings[key]} onChange={e => set(key, e.target.checked)} sx={sx.switch} />}
      ...
      <Card sx={sx.card}>
        <CardContent>
          <Typography sx={sx.sectionTitle}><FilterIcon fontSize="small" sx={{ color: '#e50914' }} />فیلتر و جستجو</Typography>
          <Stack spacing={1}>
            {[
              { key: 'enableFilter', label: 'فعال بودن فیلتر ژانر / سال / امتیاز' },
              { key: 'enableSearch', label: 'فعال بودن جستجو در صفحه فیلم‌ها' },
            ].map(({ key, label }) => (
              <FormControlLabel
                key={key}
                control={<Switch checked={settings[key]} onChange={e => set(key, e.target.checked)} sx={sx.switch} />}
      ...
      <Card sx={sx.card}>
        <CardContent>
          <Typography sx={sx.sectionTitle}><SortIcon fontSize="small" sx={{ color: '#e50914' }} />مرتب‌سازی و صفحه‌بندی</Typography>
          <Stack spacing={3}>
            <FormControl fullWidth sx={sx.input}>
              <InputLabel>مرتب‌سازی پیش‌فرض</InputLabel>
              <Select value={settings.defaultSort} label="مرتب‌سازی پیش‌فرض" onChange={e => set('defaultSort', e.target.value)}
                ...
                <MenuItem value="name">نام فیلم (الفبا)</MenuItem>
              </Select>
            </FormControl>
            <Box>
              <Typography sx={{ ...sx.label, mb: 1 }}>
                تعداد فیلم در هر صفحه: <Chip label={settings.perPage} size="small" sx={{ background: '#e50914', color: '#fff', mr: 1 }} />
              </Typography>
              <Slider value={settings.perPage} min={8} max={60} step={4}
                marks={[{ value: 8, label: '۸' }, { value: 20, label: '۲۰' }, { value: 40, label: '۴۰' }, { value: 60, label: '۶۰' }]}
                onChange={(_, v) => set('perPage', v)}
                sx={{ color: '#e50914', '& .MuiSlider-markLabel': { color: '#64748b', fontFamily: 'IRANSans, Vazirmatn, sans-serif', fontSize: '0.75rem' } }}
              />
            </Box>
          </Stack>
        </CardContent>
